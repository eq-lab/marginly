...
const exportToJsonCommand = new Command('export-to-json')
  ...
  .action(async ({ fileName }: { fileName: string }, command: Command) => {
    ...
    const ethKey = await readParameterInteractive(ethKeyParameter, systemContext);
    const wallet = new ethers.Wallet(ethKey);
    const password = await readParameterInteractive(passwordParameter, systemContext);
