...
describe('burn()', function () {
  ...
  it('should require owner', async function () {
    ...
    await expect(admin.burn(1, 1)).to.be.rejectedWith('burn amount exceeds balance');
    await expect(user.burn(1, 1)).to.be.rejectedWith('burn amount exceeds balance');
  ...
  it('should throw error when amount is zero', async function () {
    ...
    await expect(owner.burn(1, 0)).to.be.rejectedWith('invalid amount');
  });
  ...
  it('should throw error when amount exceeds balance', async function () {
    ...
    await expect(owner.burn(1, 2)).to.be.rejectedWith('burn amount exceeds balance');
  });
