...
// @ts-ignore
import { abi, bytecode } from '@marginly/router/artifacts/contracts/adapters/RamsesAdapter.sol/RamsesAdapter.json';
import { PromiseOrValue } from '../utils/api-gen';

export interface RamsesAdapterInterface extends utils.Interface {
  functions: {
    ...
    'ramsesV2SwapCallback(int256,int256,bytes)': utils.FunctionFragment;
...
export interface RamsesAdapterContract extends BaseContract {
  connect(signerOrProvider: Signer | providers.Provider | string): this;
  attach(addressOrName: string): this;
  deployed(): Promise<this>;

  interface: RamsesAdapterInterface;
...
export async function deploy(
  ...
  signer?: Signer
): Promise<RamsesAdapterContract> {
  const factory = new ContractFactory(abi, bytecode, signer);
  const contract = await factory.deploy(pools);
  return (await contract.deployed()) as any;
}

export function connect(addressOrName: string, signerOrProvider?: Signer | providers.Provider): RamsesAdapterContract {
