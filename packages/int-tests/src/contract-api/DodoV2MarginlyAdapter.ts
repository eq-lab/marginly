...
// @ts-ignore
import { abi, bytecode } from '@marginly/router/artifacts/contracts/adapters/DodoV2Adapter.sol/DodoV2Adapter.json';
import { PromiseOrValue } from '../utils/api-gen';

export interface DodoV2AdapterInterface extends utils.Interface {
  functions: {
...
export interface DodoV2AdapterContract extends BaseContract {
  connect(signerOrProvider: Signer | providers.Provider | string): this;
  attach(addressOrName: string): this;
  deployed(): Promise<this>;

  interface: DodoV2AdapterInterface;
...
export async function deploy(
  ...
  signer?: Signer
): Promise<DodoV2AdapterContract> {
  const factory = new ContractFactory(abi, bytecode, signer);
  const contract = await factory.deploy(pools);
  return (await contract.deployed()) as any;
}

export function connect(addressOrName: string, signerOrProvider?: Signer | providers.Provider): DodoV2AdapterContract {
