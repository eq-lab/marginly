...
export async function balanceSyncWithdrawBase(sut: SystemUnderTest) {
  ...
  await(await marginlyPool
    .connect(lender).execute(CallType.DepositBase, baseTransferAmount, 0, false, ZERO_ADDRESS, { gasLimit: 500_000 })).wait();
  ...
  await(await marginlyPool
    .connect(shorter).execute(CallType.DepositQuote, quoteTransferAmount, baseTransferAmount, false, ZERO_ADDRESS, { gasLimit: 500_000 })).wait();
  ...
  await(await (marginlyPool.connect(lender).execute(CallType.WithdrawBase, baseTransferAmount, 0, false, ZERO_ADDRESS, { gasLimit: 500_000 }))).wait();
...
export async function balanceSyncWithdrawQuote(sut: SystemUnderTest) {
  ...
  logger.info(`Lender deposits`);
  await(await marginlyPool
    .connect(lender).execute(CallType.DepositQuote, quoteTransferAmount, 0, false, ZERO_ADDRESS, { gasLimit: 500_000 })).wait();
  ...
  logger.info(`Long`);
  await(await marginlyPool
    .connect(longer).execute(CallType.DepositBase, baseTransferAmount, baseTransferAmount, false, ZERO_ADDRESS, { gasLimit: 500_000 })).wait();
  ...
  logger.info(`Lender withdraws`);
  await(await (marginlyPool.connect(lender).execute(CallType.WithdrawQuote, quoteTransferAmount, 0, false, ZERO_ADDRESS, { gasLimit: 500_000 }))).wait();
  logger.info(`Lender withdrew, but that tx should've failed`);
