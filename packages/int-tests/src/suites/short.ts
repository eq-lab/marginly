...
import { CallType, assertAccruedRateCoeffs, decodeSwapEvent, getShortSortKeyX48, uniswapV3Swapdata } from '../utils/chain-ops';
...
export async function short(sut: SystemUnderTest) {
  ...
  for (let i = 0; i < shortersNumber; ++i) {
    ...
    await gasReporter.saveGasUsage(
      ...
      marginlyPool
        .connect(shorter)
        .execute(CallType.DepositQuote, initCollateral, 0, false, ZERO_ADDRESS, uniswapV3Swapdata(), { gasLimit: 700_000 })
    );
