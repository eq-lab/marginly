...
export async function deleveragePrecisionLong(sut: SystemUnderTest) {
  ...
  for (let i = 0; i < 10; ++i) {
    ...
    const quoteCollateralDelevCoeffBefore = BigNumber.from(await marginlyPool.quoteCollateralDelevCoeff());
    const baseDebtDelevCoeffBefore = BigNumber.from(await marginlyPool.baseDebtDelevCoeff());

    console.log(`USDC Balance before liquidation ${formatUnits(await usdc.balanceOf(marginlyPool.address), 6)}`);
    console.log(`WETH Balance before liquidation ${formatUnits(await weth.balanceOf(marginlyPool.address), 18)}`);
    ...
    const quoteCollateralDelevCoeffAfter = BigNumber.from(await marginlyPool.quoteCollateralDelevCoeff());
    const baseDebtDelevCoeffAfter = BigNumber.from(await marginlyPool.baseDebtDelevCoeff());

    assert(!quoteCollateralDelevCoeffBefore.eq(quoteCollateralDelevCoeffAfter));
    assert(!baseDebtDelevCoeffBefore.eq(baseDebtDelevCoeffAfter));
...
export async function deleveragePrecisionShort(sut: SystemUnderTest) {
  ...
  for (let i = 0; i < 10; ++i) {
    ...
    const baseCollateralDelevCoeffBefore = BigNumber.from(await marginlyPool.baseCollateralDelevCoeff());
    const quoteDebtDelevCoeffBefore = BigNumber.from(await marginlyPool.quoteDebtDelevCoeff());
    ...
    const baseCollateralDelevCoeffAfter = BigNumber.from(await marginlyPool.baseCollateralDelevCoeff());
    const quoteDebtDelevCoeffAfter = BigNumber.from(await marginlyPool.quoteDebtDelevCoeff());

    assert(!baseCollateralDelevCoeffBefore.eq(baseCollateralDelevCoeffAfter));
    assert(!quoteDebtDelevCoeffBefore.eq(quoteDebtDelevCoeffAfter));
    logger.info(`  Liquidation happened`);
    logger.info(`  baseCollateralDelevCoeffAfter = ${baseCollateralDelevCoeffAfter}`);
    logger.info(`  quoteDebtDelevCoeffAfter = ${quoteDebtDelevCoeffAfter}`);
...
async function addToLogs(
  ...
  const baseDebtDelevCoeff = BigNumber.from(await marginlyPool.baseDebtDelevCoeff());
  const quoteCollateralCoeff = BigNumber.from(await marginlyPool.quoteCollateralCoeff());
  const quoteCollateralDelevCoeff = BigNumber.from(await marginlyPool.quoteCollateralDelevCoeff());

  const baseCollateralCoeff = BigNumber.from(await marginlyPool.baseCollateralCoeff());
  const baseCollateralDelevCoeff = BigNumber.from(await marginlyPool.baseCollateralDelevCoeff());
  const quoteDebtCoeff = BigNumber.from(await marginlyPool.quoteDebtCoeff());
  const quoteDebtDelevCoeff = BigNumber.from(await marginlyPool.quoteDebtDelevCoeff());
  ...
  coeffsTable[transactionName] = {
    ...
    quoteCollateralDelevCoeff: quoteCollateralDelevCoeff.toString(),
    quoteCollateralDelevCoeffHuman: toHumanString(quoteCollateralDelevCoeff),
    baseDebtCoeff: baseDebtCoeff.toString(),
    baseDebtCoeffHuman: toHumanString(baseDebtCoeff),
    baseDebtDelevCoeff: baseDebtDelevCoeff.toString(),
    baseDebtDelevCoeffHuman: toHumanString(baseDebtDelevCoeff),
  };
  ...
  for (let i = 0; i < lendersNum; ++i) {
    ...
    const positionCollateralDelevCoeff = position.collateralDelevCoeff;
    const positionDebtDelevCoeff = position.debtDelevCoeff;
    const realBaseCollateral = baseCollateralCoeff.mul(discountedBaseCollateral).div(FP96.one);
    const realQuoteCollateral = quoteCollateralCoeff.mul(discountedQuoteCollateral).div(FP96.one);
    ...
    positionsInfo.set(`lender_${i} positionCollateralDelevCoeff`, positionCollateralDelevCoeff.toString());
    positionsInfo.set(`lender_${i} positionDebtDelevCoeff`, positionDebtDelevCoeff.toString());
  }
  ...
  for (let i = 0; i < longersNum; ++i) {
    ...
    const positionCollateralDelevCoeff = position.collateralDelevCoeff.inner;
    const positionDebtDelevCoeff = position.debtDelevCoeff.inner;
    // const realBaseCollateral =
    //   position._type == 3
    //     ? baseCollateralCoeff
    //         .mul(
    //           BigNumber.from(discountedBaseCollateral).mul(baseCollateralDelevCoeff).div(positionCollateralDelevCoeff)
    //         )
    //         .div(FP96.one)
    //     : baseCollateralCoeff.mul(discountedBaseCollateral).div(FP96.one);

    // const realQuoteDebt =
    //   position._type == 3
    //     ? quoteDebtCoeff
    //         .mul(BigNumber.from(discountedQuoteDebt).mul(quoteDebtDelevCoeff).div(positionDebtDelevCoeff))
    //         .div(FP96.one)
    //     : quoteDebtCoeff.mul(discountedQuoteDebt).div(FP96.one);
    ...
    positionsInfo.set(`longer_${i} positionCollateralDelevCoeff`, positionCollateralDelevCoeff.toString());
    positionsInfo.set(`longer_${i} positionDebtDelevCoeff`, positionDebtDelevCoeff.toString());
  }
  ...
  for (let i = 0; i < shortersNum; ++i) {
    ...
    //   .mul(BigNumber.from(discountedQuoteCollateral).mul(quoteCollateralDelevCoeff).div(positionCollateralDelevCoeff))
    //   .div(FP96.one);

    // const realBaseDebt = baseDebtCoeff
    //   .mul(BigNumber.from(discountedBaseDebt).mul(baseDebtDelevCoeff).div(positionDebtDelevCoeff))
    //   .div(FP96.one);
    ...
    //         .mul(
    //           BigNumber.from(discountedQuoteCollateral).mul(quoteCollateralDelevCoeff).div(positionCollateralDelevCoeff)
    //         )
    //         .div(FP96.one)
    //     : quoteCollateralCoeff.mul(discountedQuoteCollateral).div(FP96.one);
    ...
    //         .mul(BigNumber.from(discountedBaseDebt).mul(baseDebtDelevCoeff).div(positionDebtDelevCoeff))
    //         .div(FP96.one)
    //     : baseDebtCoeff.mul(discountedBaseDebt).div(FP96.one);
    ...
    positionsInfo.set(`shorter_${i} positionCollateralDelevCoeff`, positionCollateralDelevCoeff.toString());
    positionsInfo.set(`shorter_${i} positionDebtDelevCoeff`, positionDebtDelevCoeff.toString());
  }
