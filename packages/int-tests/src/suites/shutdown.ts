...
export async function shortEmergency(sut: SystemUnderTest) {
  ...
  await (
    await marginlyPool
      .connect(lender)
      .execute(CallType.DepositBase, lenderDepositBaseAmount, 0, false, ZERO_ADDRESS, { gasLimit: 400_000 })
  ).wait();
  ...
  for (const shorter of shorters) {
    ...
    await (
      await marginlyPool
        .connect(shorter)
        .execute(CallType.DepositQuote, shorterDepositQuote, shortAmount, false, ZERO_ADDRESS, { gasLimit: 900_000 })
    ).wait();
  }
  await showSystemAggregates(sut);
  ...
  await (
    await marginlyPool
      .connect(longer)
      .execute(CallType.DepositBase, longDepositBase, 0, false, ZERO_ADDRESS, { gasLimit: 400_000 })
  ).wait();
  ...
  await (
    await marginlyPool.connect(longer).execute(CallType.Long, longAmount, 0, false, ZERO_ADDRESS, { gasLimit: 900_000 })
  ).wait();
  await showSystemAggregates(sut);
  ...
  for (let i = 0; i < 30; i++) {
    ...
    try {
      const txReceipt = await (
        await marginlyPool.connect(treasury).execute(CallType.Reinit, 0, 0, false, ZERO_ADDRESS, { gasLimit: 1_500_000 })
      ).wait();
  ...
  await (
    await marginlyPool
      .connect(longer)
      .execute(CallType.EmergencyWithdraw, 0, 0, false, ZERO_ADDRESS, { gasLimit: 400_000 })
  ).wait();
  await (
    await marginlyPool
      .connect(lender)
...
export async function longEmergency(sut: SystemUnderTest) {
  ...
  await (
    await marginlyPool
      .connect(lender)
      .execute(CallType.DepositQuote, lenderDepositQuoteAmount, 0, false, ZERO_ADDRESS, { gasLimit: 400_000 })
  ).wait();
  await showSystemAggregates(sut);
  ...
  for (const longer of longers) {
    ...
    await (
      await marginlyPool
        .connect(longer)
        .execute(CallType.DepositBase, longDepositBase, longAmount, false, ZERO_ADDRESS, { gasLimit: 900_000 })
    ).wait();
  }
  await showSystemAggregates(sut);
  ...
  await (
    await marginlyPool
      .connect(shorter)
      .execute(CallType.DepositQuote, shorterDepositQuote, 0, false, ZERO_ADDRESS, { gasLimit: 900_000 })
  ).wait();
  ...
  await (
    await marginlyPool
      .connect(shorter)
      .execute(CallType.Short, shortAmount, 0, false, ZERO_ADDRESS, { gasLimit: 900_000 })
  ).wait();
  logger.info(`Short to ${formatUnits(shortAmount, 18)} WETH`);
  await showSystemAggregates(sut);
