  ...
  for (let i = 0; i < 30; i++) {
    ...
    try {
      ...
      await (
        await marginlyPool
          .connect(receiver)
          .execute(CallType.ReceivePosition, quoteAmount, wethAmount, false, shorter.address, { gasLimit: 300_000 })
      ).wait();
  ...
  for (let i = 0; i < 30; i++) {
    ...
    try {
      ...
      await (
        await marginlyPool
          .connect(liquidator)
          .execute(CallType.ReceivePosition, quoteAmount, wethAmount, false, longer.address, { gasLimit: 300_000 })
      ).wait();
