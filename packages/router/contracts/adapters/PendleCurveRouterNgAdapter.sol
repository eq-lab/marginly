...
contract PendleCurveRouterNgAdapter is IMarginlyAdapter, Ownable2Step {
  ...
  function _addPairs(RouteInput[] memory routes) private {
    ...
    for (uint256 i; i < length; ) {
      ...
      if (input.curveDxAdjustPtToToken >= CURVE_DX_ADJUST_ONE) revert WrongInput();
      if (input.curveDxAdjustPtToToken + CURVE_DX_ADJUST_ONE < 0) revert WrongInput();
      if (input.curveDxAdjustTokenToPt >= CURVE_DX_ADJUST_ONE) revert WrongInput();
      if (input.curveDxAdjustTokenToPt + CURVE_DX_ADJUST_ONE < 0) revert WrongInput();
