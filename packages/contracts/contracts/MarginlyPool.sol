...
contract MarginlyPool is IMarginlyPool {
  ...
  function depositBase(uint256 amount) external override lock {
    ...
    if (position._type == PositionType.Short) {
      ...
      if (amount >= realBaseDebt) {
        ...
        discountedBaseDebt = discountedBaseDebt.sub(discountedBaseDelta);
      } else {
        // Short position, debt > depositAmount, decrease debt
        // discountedBaseDebtDelta = (realDebt - amount) / coeff
        uint256 discountedBaseDebtDelta = baseDebtCoeff.recipMul(realBaseDebt.sub(amount));
  ...
  function depositQuote(uint256 amount) external override lock {
    ...
    if (position._type == PositionType.Long) {
      ...
      if (amount >= realQuoteDebt) {
        ...
        uint256 discountedQuoteDebtDelta = quoteDebtCoeff.recipMul(realQuoteDebt.sub(amount));
  ...
  function closePosition() external override lock {
    ...
    if (position._type == PositionType.Short) {
      ...
      uint256 exactQuoteIn = realQuoteDebt.add(realFeeAmount);
      uint256 swappedBaseCollateral = swapExactOutput(false, realBaseCollateral, exactQuoteIn);

      //Check slippage below params.positionSlippage
      uint256 baseInMaximum = FP96.fromRatio(WHOLE_ONE + params.positionSlippage, WHOLE_ONE).mul(
        getCurrentBasePrice().recipMul(exactQuoteIn)
      );
    ...
    emit ClosePosition(msg.sender, collateralToken, realCollateralDelta);
  }
  ...
  function short(uint256 realBaseAmount) external override lock {
    ...
    emit Short(msg.sender, realBaseAmount);
  }
  ...
  function long(uint256 realBaseAmount) external override lock {
    ...
    uint256 discountedBaseCollateralChange = baseCollateralCoeff.recipMul(realBaseAmount);
    ...
    emit Long(msg.sender, realBaseAmount);
  }
