...
contract MarginlyPool is IMarginlyPool {
  ...
  function depositBase(
    ...
    if (newPoolBaseBalance(amount) > basePrice.recipMul(params.quoteLimit)) revert Errors.ExceedsLimit();
  ...
  function long(
    ...
    if (realBaseAmount < params.positionMinAmount) revert Errors.LessThanMinimalAmount();
    if (newPoolBaseBalance(realBaseAmount) > basePrice.recipMul(params.quoteLimit)) revert Errors.ExceedsLimit();
