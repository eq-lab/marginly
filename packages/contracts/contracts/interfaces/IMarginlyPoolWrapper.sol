...
interface IMarginlyPoolWrapper {
    ...
    function long(uint32 poolIndex, uint256 depositBaseAmount, uint256 longBaseAmount) external;
    
    /// @dev wrapped Marginly depositQuote and short calls. Works for creating new positions only
    function short(uint32 poolIndex, uint256 depositQuoteAmount, uint256 shortBaseAmount) external;
}