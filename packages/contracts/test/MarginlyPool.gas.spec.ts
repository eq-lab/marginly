...
describe('Open position:', () => {
  it('depositBase', async () => {
    ...
    await snapshotGasCost(await marginlyPool.connect(signer).execute(CallType.DepositBase, depositAmount, 0, false, ZERO_ADDRESS));
  });
  ...
  it('depositQuote', async () => {
    ...
    await snapshotGasCost(await marginlyPool.connect(signer).execute(CallType.DepositQuote, depositAmount, 0, false, ZERO_ADDRESS));
  });
});
...
describe('Deposit into existing position:', () => {
  it('depositBase', async () => {
    ...
    await snapshotGasCost(marginlyPool.connect(signer1).execute(CallType.DepositBase, secondDeposit, 0, false, ZERO_ADDRESS));
  });
  ...
  it('depositQuote', async () => {
    ...
    await snapshotGasCost(marginlyPool.connect(signer1).execute(CallType.DepositQuote, secondDeposit, 0, false, ZERO_ADDRESS));
  });
});
...
describe('mc happens:', async () => {
  it('depositBase with one mc', async () => {
    ...
    await snapshotGasCost(await marginlyPool.connect(depositor).execute(CallType.DepositBase, 1000, 0, false, ZERO_ADDRESS));
  ...
  it('depositQuote with one mc', async () => {
    ...
    await marginlyPool.connect(longer).execute(CallType.Long, longAmount, 0, false, ZERO_ADDRESS);
    await time.increase(24 * 60 * 60);
    await snapshotGasCost(await marginlyPool.connect(depositor).execute(CallType.DepositQuote, 1000, 0, false, ZERO_ADDRESS));
    expect(await marginlyPool.discountedQuoteDebt()).to.be.equal(BigNumber.from(0));
  });
  ...
  it('long closePosition with one mc', async () => {
    ...
    await snapshotGasCost(await marginlyPool.connect(longer2).execute(CallType.ClosePosition, 0, 0, false, ZERO_ADDRESS));
  ...
  it('depositBase with two mc', async () => {
    ...
    await marginlyPool.connect(shorter).execute(CallType.Short, shortAmount, 0, false, ZERO_ADDRESS);
    const longAmount = Math.floor((lev - 1) * 1000);
    await marginlyPool.connect(lender).execute(CallType.DepositQuote, Math.floor(20 * longAmount * price), 0, false, ZERO_ADDRESS);
    await marginlyPool.connect(longer).execute(CallType.Long, longAmount, 0, false, ZERO_ADDRESS);
...
describe('Liquidation', () => {
  it('liquidate long position and create new position', async () => {
    ...
    const baseAmount = 0;
    await snapshotGasCost(marginlyPool.connect(receiver).execute(CallType.ReceivePosition, quoteAmount, baseAmount, false, longer.address));
  });
  ...
  it('liquidate short position and create new position', async () => {
    ...
    await snapshotGasCost(marginlyPool.connect(receiver).execute(CallType.ReceivePosition, quoteAmount, baseAmount, false, shorter.address));
  });
