...
describe('MarginlyPool.Base', () => {
  ...
  describe('Deposit base', async () => {
    ...
    it('first deposit should create position', async () => {
      ...
      const expectedDBC = convertFP96ToNumber((await marginlyPool.baseCollateralCoeff()).inner) * depositAmount;
    ...
    it('different signers deposits', async () => {
      ...
      const expectedDBC1 = convertFP96ToNumber((await marginlyPool.baseCollateralCoeff()).inner) * firstDeposit;
      const expectedDBC2 = convertFP96ToNumber((await marginlyPool.baseCollateralCoeff()).inner) * secondDeposit;

      expect(await marginlyPool.discountedBaseCollateral()).to.be.equal(expectedDBC1 + expectedDBC2);
      const first = await marginlyPool.getPosition(signer1.address);
      expect(first.discountedBaseAmount).to.be.equal(expectedDBC1);

      const second = await marginlyPool.getPosition(signer2.address);
      expect(second.discountedBaseAmount).to.be.equal(expectedDBC2);
    });
    ...
    it('deposit into positive base position', async () => {
      ...
      const expectedDBC = convertFP96ToNumber((await marginlyPool.baseCollateralCoeff()).inner) * total;
    ...
    it('deposit into short position', async () => {
      ...
      let position = await marginlyPool.getPosition(signer.address);
      ...
      position = await marginlyPool.getPosition(signer.address);
    ...
    it('deposit into long position', async () => {
      ...
      const positionBefore = await marginlyPool.getPosition(signer.address);
      ...
      const positionAfter = await marginlyPool.getPosition(signer.address);
  ...
  describe('Deposit quote', async () => {
    ...
    it('first deposit should create position', async () => {
      ...
      const expectedDQC = convertFP96ToNumber((await marginlyPool.quoteCollateralCoeff()).inner) * depositAmount;
      ...
      const position = await marginlyPool.getPosition(signer.address);
      expect(position._type).to.be.equal(PositionType.Lend);
      expect(position.discountedQuoteAmount).to.be.equal(expectedDQC); //quote
      expect(position.discountedBaseAmount).to.be.equal(0); //base
    });
    ...
    it('deposit into positive quote position', async () => {
      ...
      const expectedDQC = convertFP96ToNumber((await marginlyPool.quoteCollateralCoeff()).inner) * total;
  ...
  describe('Withdraw base', () => {
    ...
    it('should decrease base position', async () => {
      ...
      const expectedDBC = convertFP96ToNumber((await marginlyPool.baseCollateralCoeff()).inner) * expectedRBC;
      const expectedDQC = convertFP96ToNumber((await marginlyPool.quoteCollateralCoeff()).inner) * amountToDeposit;
  ...
  describe('Withdraw quote', () => {
    ...
    it('should decrease quote position', async () => {
      ...
      const expectedDQC = convertFP96ToNumber((await marginlyPool.quoteCollateralCoeff()).inner) * expectedRQC;
      const expectedDBC = convertFP96ToNumber((await marginlyPool.baseCollateralCoeff()).inner) * amountToDeposit;
    ...
    it('reinit', async () => {
      ...
      const baseDebtCoeff = (await marginlyPool.baseDebtCoeff()).inner;
      const quoteDebtCoeff = (await marginlyPool.quoteDebtCoeff()).inner;
  ...
  describe('Short', () => {
    ...
    it('short should update leverageShort', async () => {
      ...
      const position = await marginlyPool.getPosition(shorter.address);
    ...
    it('short, changed from lend to short', async () => {
      ...
      const expectedDBC0 = convertFP96ToNumber((await marginlyPool.baseCollateralCoeff()).inner) * amountToDeposit;
      const expectedDQC0 = convertFP96ToNumber((await marginlyPool.quoteCollateralCoeff()).inner) * amountToDeposit;
      ...
      const shorterPositionBefore = await marginlyPool.getPosition(shorter.address);
      ...
      const expectedDBC1 = convertFP96ToNumber((await marginlyPool.baseCollateralCoeff()).inner) * amountToDeposit;
      const expectedRQC1 = BigNumber.from(amountToDeposit).add(
        BigNumber.from((1 - swapFee) * shortAmount)
          .mul(price)
          .div(one)
      );
      const expectedDQC1 = (await marginlyPool.quoteCollateralCoeff()).inner.mul(expectedRQC1).div(one);
      const debtCoeff = (await marginlyPool.baseDebtCoeff()).inner;
    ...
    it('short, update short position', async () => {
      ...
      const expectedRQC1 =
        ...
        +BigNumber.from((1 - swapFee) * shortAmount)
          .mul(price)
          .div(one);
      const expectedDQC1 = convertFP96ToNumber((await marginlyPool.quoteCollateralCoeff()).inner) * expectedRQC1;
      const debtCoeff1 = (await marginlyPool.baseDebtCoeff()).inner;
      ...
      const expectedDBC2 = convertFP96ToNumber((await marginlyPool.baseCollateralCoeff()).inner) * amountToDeposit;
      const expectedRQC2 =
        amountToDeposit +
        +BigNumber.from((1 - swapFee) * totalShortAmount)
          .mul(price)
          .div(one);
      const expectedDQC2 = convertFP96ToNumber((await marginlyPool.quoteCollateralCoeff()).inner) * expectedRQC2;
      const debtCoeff2 = (await marginlyPool.baseDebtCoeff()).inner;
      ...
      const shorterPositionAfterUpdate = await marginlyPool.getPosition(shorter.address);
  ...
  describe('Long', () => {
    ...
    it('long should update leverageLong', async () => {
      ...
      const position = await marginlyPool.getPosition(longer.address);
      const basePrice = await marginlyPool.getBasePrice();
      const initialPrice = await marginlyPool.initialPrice();
    ...
    it('changed from lend to long', async () => {
      ...
      const longerPositionBefore = await marginlyPool.getPosition(longer.address);
      ...
      const expectedDBC1 = (await marginlyPool.baseCollateralCoeff()).inner.mul(expectedRBC1).div(one);
      const expectedDQC1 = (await marginlyPool.quoteCollateralCoeff()).inner
        .mul(BigNumber.from(amountToDeposit))
        .div(one);
      const debtCoeff = (await marginlyPool.quoteDebtCoeff()).inner;
    ...
    it('update long position', async () => {
      ...
      const expectedDBC1 = convertFP96ToNumber((await marginlyPool.baseCollateralCoeff()).inner) * expectedRBC1;
      const expectedDQC1 = convertFP96ToNumber((await marginlyPool.quoteCollateralCoeff()).inner) * amountToDeposit;
      const debtCoeff = (await marginlyPool.quoteDebtCoeff()).inner;
      ...
      const expectedDBC2 = convertFP96ToNumber((await marginlyPool.baseCollateralCoeff()).inner) * expectedRBC2;
      const expectedDQC2 = convertFP96ToNumber((await marginlyPool.quoteCollateralCoeff()).inner) * amountToDeposit;
      const debtCoeff2 = (await marginlyPool.quoteDebtCoeff()).inner;
      ...
      const longerPositionAfterUpdate = await marginlyPool.getPosition(longer.address);
  ...
  describe('Position sort keys', () => {
    it('should properly calculate sort key for long position', async () => {
      ...
      const position1 = await marginlyPool.getPosition(longer1.address);
    ...
    it('should properly calculate sort key for short position', async () => {
      ...
      const position1 = await marginlyPool.getPosition(shorter1.address);
    ...
    it('long position sortKey', async () => {
      ...
      const position1 = await marginlyPool.getPosition(longer1.address);
      const position2 = await marginlyPool.getPosition(longer2.address);
    ...
    it('short position sortKey', async () => {
      ...
      let position1 = await marginlyPool.getPosition(shorter1.address);
      let position2 = await marginlyPool.getPosition(shorter2.address);
      ...
      position1 = await marginlyPool.getPosition(shorter1.address);
      position2 = await marginlyPool.getPosition(shorter2.address);
      const position3 = await marginlyPool.getPosition(shorter3.address);
  ...
  describe('Transfer position', () => {
    it('success', async () => {
      ...
      const positionBefore = await marginlyPool.getPosition(from.address);

      await marginlyPool.connect(from).transferPosition(to.address);

      const positionAfter = await marginlyPool.getPosition(from.address);
      const newPosition = await marginlyPool.getPosition(to.address);
