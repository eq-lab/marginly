  ...
  Ownable
{
  ...
  function setPool(address tokenA, address tokenB, uint24 fee, address pool) external onlyOwner {
    (address token0, address token1) = sortTokens(tokenA, tokenB);
    pools[token0][token1][fee] = pool;
  }

  function verifyCallback(address tokenA, address tokenB, uint24 fee) internal view returns (IUniswapV3Pool pool) {
