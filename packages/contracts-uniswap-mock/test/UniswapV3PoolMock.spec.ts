...
async function createTestUniswapV3PoolMock(
  oracle: string,
  tokenA: string,
  tokenB: string,
  fee: number
): Promise<TestUniswapV3PoolMock> {
...
async function setPrice(
  ...
  oracle: SignerWithAddress,
  tokens: [MintableToken | WETH9, MintableToken | WETH9],
  price: number
) {
  ...
  priceHistories.forEach(({ prices, tokenPair: [baseTokenKey, quoteTokenKey], tests }, priceHistoryNum) =>
    tests.forEach(({ secondsAgos, expectedPrice }) =>
      it(`should calc observations properly for time range [${secondsAgos[0]}, ${secondsAgos[1]}] in prices history ${priceHistoryNum}`, async () => {
        const { pool, tokens, owner, oracle } = await loadFixture(createContracts);
        ...
        const tickCumulatives: [bigint, bigint] = [
          observations.tickCumulatives[0].toBigInt(),
          observations.tickCumulatives[1].toBigInt(),
        ];

        const [[token0Decimals, isToken0Base], [token1Decimals]] = sortUniswapPoolTokens(
          [baseToken.address as `0x${string}`, quoteToken.address as `0x${string}`],
          [
            [await baseToken.decimals(), true],
            [await quoteToken.decimals(), false],
          ]
        );
        let actualPrice = twapFromTickCumulatives(
          tickCumulatives,
          [BigInt(secondsAgos[0]), BigInt(secondsAgos[1])],
          token0Decimals,
          token1Decimals
        );
...
describe('UniswapV3FactoryMock test', () => {
  it('Factory creates pool', async () => {
    ...
    const poolAddress = creationEvent!.args![4];
