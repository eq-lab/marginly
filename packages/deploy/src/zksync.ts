...
function createHreMock(l2Url: string, l1Url: string) {
  return {
    network: {
      name: 'default',
      zksync: true,
...
export function createDeployer(wallet: Wallet) {
  const l2Url = wallet.provider.connection.url;
  const l1Url = (wallet.providerL1 as Provider | undefined)?.connection?.url;

  if (l1Url === undefined) {
    throw new Error('Unable to read L1 node url');
  }

  const hreMock = createHreMock(l2Url, l1Url);
