...
describe('safeTransferFrom()', function () {
  ...
  it('should require owner of token', async function () {
    ...
    await expect(
      admin.safeTransferFrom(this.signers.users[0].address, this.signers.users[1].address, 1, 23, "0x")
    ).to.be.rejectedWith('ERC1155: caller is not token owner or approved');
  });
  ...
  it('should fail when insufficient token balance', async function () {
    ...
    await expect(
      user.safeTransferFrom(this.signers.users[0].address, this.signers.users[1].address, 1, 200, "0x")
    ).to.be.rejectedWith('ERC1155: insufficient balance for transfer');
  });

  it('should transfer token amount and emit TransferSingle event', async function () {
    const admin = ContestWinnerNFT__factory.connect(await this.contestWinnerNft.getAddress(), this.signers.admin);
    const user = ContestWinnerNFT__factory.connect(await this.contestWinnerNft.getAddress(), this.signers.users[0]);

    await admin.mint([this.signers.users[0].address], [1], [123]);
    await expect(user.safeTransferFrom(this.signers.users[0].address, this.signers.users[1].address, 1, 23, "0x"))
